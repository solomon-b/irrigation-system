...
run:
  cabal run exe:irrigation-web-server
...
# Build Arduino controller code
arduino-build:
  nix run .#arduino-build -- controller
...
migrations-add MIGRATION:
  sqlx migrate add {{MIGRATION}} --source web-server/migrations

# Run SQL migrations.
migrations-run:
  sqlx migrate run --source web-server/migrations

# Reset PG Database.
migrations-reset:
  sqlx database reset --source web-server/migrations
...
postgres-dev-stop:
  ...
  docker container stop "dev-postgres-{{PROJECT}}"
  echo ✨ "Success!"
...
postgres-test-start:
  ...
  docker run --rm --name "test-postgres-{{PROJECT}}" -d -p 5432:5432 -e POSTGRES_HOST_AUTH_METHOD=trust -e POSTGRES_DB=test_db -d postgres
  echo "✨ Success!"

# Halt the test docker container
postgres-test-stop:
  echo "🔴 Stopping the Test Postgres service.."
  docker container stop "test-postgres-{{PROJECT}}"
  echo ✨ "Success!"
